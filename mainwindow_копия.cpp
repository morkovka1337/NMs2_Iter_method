...
void MainWindow::on_pushButton_clicked()
{
    ...
    for (unsigned int i = 0; i < n - 1; ++i)
    {
        for (unsigned int j = 0; j < m -1; ++j)
        {
            ...
            if (i == 0)
            {
                f[i][j] -= myu1_test(c + (j+1) * k) / (k * k);
            }
            if (i == (n-2))
            {
                f[i][j] -= myu2_test(c + (j+1) * k) / (k * k);
            }
            if (j == 0)
            {
                f[i][j] -= myu3_test(a + (i+1) * h) / (h * h);
            }
            if (j == (m-2))
            {
                f[i][j] -= myu4_test(a + (i+1) * h) / (h * h);
            }
        }
    }
//    for (int j = 0; j <= m; ++j)
...
//    }
    ...
    {
        solution = solveConjugateGradient(startSolution, f, n, m, limit, eps);
    }
    ...
    for (unsigned int i = 0; i < n+1; ++i)
    {
        for (unsigned int j = 0; j < m+1; ++j)
        {
            if (i > 0 && i < n && j > 0 && j < m)
            {
            ui->tableWidget->setItem(i, j, new QTableWidgetItem(QString::number(solution[i-1][j-1])));
            }
            else if (i == 0)
            {
                 ui->tableWidget->setItem(i, j, new QTableWidgetItem(QString::number(myu1_test(c + j * k))));
            }
            else if (i == n)
            {
                 ui->tableWidget->setItem(i, j, new QTableWidgetItem(QString::number(myu2_test(c + j * k))));
            }
            else if (j == 0)
            {
                 ui->tableWidget->setItem(i, j, new QTableWidgetItem(QString::number(myu3_test(a + i * h))));
            }
            else if (j == m)
            {
                 ui->tableWidget->setItem(i, j, new QTableWidgetItem(QString::number(myu4_test(a + i * h))));
            }
        }
    }
    auto maxDiffSol = 0.0;
    for (unsigned int i = 1; i < n ; ++i)
    {
        for (unsigned int j = 1; j < m ; ++j)
        {
            double currentElem = abs(solution[i-1][j-1] - accurateSolution[i][j]);
